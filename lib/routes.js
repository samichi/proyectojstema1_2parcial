...
module.exports = function (app) {
	...
	app.post('/capitulo/:id_excursion', function (req, res, next) { //crear
		if (req.isAuthenticated()) {
			...
			let imgcapitulo = req.body.imgcapitulo;
			let descripcion = req.body.descripcion;
			let creditos = req.body.creditos;
			let urlvideo = req.body.urlvideo;
			let urlrespuesta = req.body.urlrespuesta;
    ...
    form.on('fileBegin', function (name, file){
		file.path = 'public/audios/'+ file.name;
		audioFIle = file.path;
    });
    ...
    form.on('file', function (name, file){
        console.log('Uploaded ' + file.name);
			...
			pool.connect(function (err) {
				...
				//INSERT INTO excursion (nombre, urlimg) VALUES ('MARCO', '/URLMARCO');
				pool.query("INSERT INTO  capitulo  (titulo, imgcapitulo, descripcion, creditos, urlvideo, urlactividad, urlrespuesta, id_excursion) VALUES ('" + titulo + "', '" + imgcapitulo + "', '" + descripcion + "', '" + creditos + "', '" + urlvideo + "', '" + audioFIle + "', '" + urlrespuesta + "', " + id_excursion + ");", function (err, result) {
					if (err) {
						return console.error('Error al ejecutar el query', err);
					}
					//pool.end();
	...
	app.post('/capitulo/:id_excursion/:id', (req, res) => {
		...
		pool.connect(function (err) {
			...
			let audioFIle = req.body.urlactividad;
			let titulo = req.body.titulo;
			let imgcapitulo = req.body.imgcapitulo;
			let descripcion = req.body.descripcion;
			let creditos = req.body.creditos;
    ...
    form.on('file', function (name, file){
        console.log('Uploaded ' + file.name);
			...
			pool.query("UPDATE capitulo SET titulo = '" + titulo + "', imgcapitulo = '" + imgcapitulo + "', descripcion = '" + descripcion + "', creditos = '" + creditos + "', urlvideo = '" + urlvideo + "', urlactividad = '" + audioFIle + "', urlrespuesta = '" + urlrespuesta + "', id_excursion = " + id_excursion + " WHERE id=" + id + ";", function (err, result) {
